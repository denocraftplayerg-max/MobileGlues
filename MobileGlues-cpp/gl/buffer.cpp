...
// ============================================================================
// GL_EXT_buffer_storage - RESOLUCAO DE FUNCOES
// ============================================================================
...
static bool g_buffer_storage_resolved = false;
...
static bool mg_buffer_storage_available() {
    if (g_buffer_storage_resolved) return g_buffer_storage_ext != nullptr;
    g_buffer_storage_resolved = true;
    ...
    return g_buffer_storage_ext != nullptr;
}

// ============================================================================
// PER-SHARE-GROUP E PER-CONTEXT STORAGE (ORIGINAL)
// ============================================================================
namespace {

struct buffer_group_state_t {
...
std::unordered_map<unsigned long long, std::unique_ptr<buffer_group_state_t>> g_buf_groups;
std::unordered_map<unsigned long long, std::unique_ptr<buffer_ctx_state_t>> g_buf_ctxs;
...
enum BindingIndex : int {
    BI_ARRAY_BUFFER = 0, BI_ATOMIC_COUNTER, BI_COPY_READ, BI_COPY_WRITE,
    BI_DRAW_INDIRECT, BI_DISPATCH_INDIRECT, BI_ELEMENT_ARRAY, BI_PIXEL_PACK,
    BI_PIXEL_UNPACK, BI_SHADER_STORAGE, BI_TRANSFORM_FEEDBACK,
    BI_UNIFORM_BUFFER, BI_PARAMETER_BUFFER, BINDING_COUNT
};
...
GLuint find_bound_array() { return bound_array; }
...
static inline int binding_target_to_index(GLenum target) {
    ...
    case GL_PARAMETER_BUFFER:       return BI_PARAMETER_BUFFER;
    default: return -1;
    }
}
...
GLuint find_bound_buffer(GLenum key) {
    ...
    case GL_PARAMETER_BUFFER_BINDING:       target = GL_PARAMETER_BUFFER; break;
...
void glGenBuffers(GLsizei n, GLuint* buffers) {
    ...
    for (int i = 0; i < n; ++i) buffers[i] = gen_buffer();
}
...
void glBindBufferRange(GLenum target, GLuint index, GLuint buffer, GLintptr offset, GLsizeiptr size) {
    LOG()
    LOG_D("glBindBufferRange, target = %s, index = %d, buffer = %d, offset = %p, size = %zi",
          glEnumToString(target), index, buffer, (void*)offset, size)
    if (!has_buffer(buffer) || buffer == 0) {
...
void glBindBufferBase(GLenum target, GLuint index, GLuint buffer) {
    ...
    if (target == GL_SHADER_STORAGE_BUFFER) {
        if (g_buffer_map_ssbo_id.empty()) g_buffer_map_ssbo_id.resize(GL_MAX_SHADER_STORAGE_BUFFER_BINDINGS, 0);
...
void glBindVertexBuffer(GLuint bindingindex, GLuint buffer, GLintptr offset, GLsizei stride) {
    LOG()
    LOG_D("glBindVertexBuffer, bindingindex = %d, buffer = %d, offset = %p, stride = %i",
...
// ============================================================================
// glBufferData OTIMIZADO COM GL_EXT_buffer_storage
// ============================================================================
...
void glBufferData(GLenum target, GLsizeiptr size, const void* data, GLenum usage) {
    ...
    // Se temos GL_EXT_buffer_storage, usar para buffers frequentemente atualizados
    if (mg_buffer_storage_available() && data && size > 0) {
        bool is_streaming = (usage == GL_STREAM_DRAW || usage == GL_STREAM_COPY || usage == GL_DYNAMIC_DRAW);

        if (is_streaming && size <= 1024 * 1024) {
            // Para streaming, usar buffer persistente com double-buffering
            static thread_local GLuint g_stream_buf[2] = {0, 0};
            static thread_local int g_stream_idx = 0;
            static thread_local size_t g_stream_size = 0;

            if (g_stream_size < (size_t)size) {
                if (g_stream_buf[0]) { GLES.glDeleteBuffers(1, &g_stream_buf[0]); g_stream_buf[0] = 0; }
                if (g_stream_buf[1]) { GLES.glDeleteBuffers(1, &g_stream_buf[1]); g_stream_buf[1] = 0; }
                g_stream_size = size * 2;
            }

            int idx = g_stream_idx;
            if (!g_stream_buf[idx]) {
                GLES.glGenBuffers(1, &g_stream_buf[idx]);
                GLES.glBindBuffer(t.target, g_stream_buf[idx]);
                g_buffer_storage_ext(t.target, g_stream_size, nullptr,
                    GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT_EXT | GL_MAP_COHERENT_BIT_EXT);
            } else {
                GLES.glBindBuffer(t.target, g_stream_buf[idx]);
            }

            void* mapped = GLES.glMapBufferRange(t.target, 0, size,
                GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT_EXT | GL_MAP_COHERENT_BIT_EXT);
            if (mapped) {
...
void* glMapBuffer(GLenum target, GLenum access) {
    ...
    if (buffer_size <= 0 || glGetError() != GL_NO_ERROR) return nullptr;
    GLbitfield flags = 0;
    switch (access) {
    ...
    return glMapBufferRange(target, 0, buffer_size, flags);
}
...
void glBufferStorage(GLenum target, GLsizeiptr size, const void* data, GLbitfield flags) {
    ...
    if (mg_buffer_storage_available() && g_buffer_storage_ext) {
        ...
        g_buffer_storage_ext(t.target, size, data, flags);
        set_buffer_data_size(find_bound_buffer_by_target(target), size);
    } else {
        GLES.glBufferData(t.target, size, data, GL_STATIC_DRAW);
        set_buffer_data_size(find_bound_buffer_by_target(target), size);
    }
    CHECK_GL_ERROR
}
...
void glGenVertexArrays(GLsizei n, GLuint* arrays) {
    ...
    for (int i = 0; i < n; ++i) arrays[i] = gen_array();
}
...
static bool mg_texture_buffer_available() {
    if (g_texture_buffer_resolved) return g_tex_buffer_ext != nullptr;
    ...
    return g_tex_buffer_ext != nullptr;
}

// ============================================================================
// glTexBuffer OTIMIZADO
// ============================================================================
...
size_t get_internal_format_size(GLenum internalformat) {
    switch (internalformat) {
    case GL_R8: case GL_R8I: case GL_R8UI: return 1;
...
void glTexBuffer(GLenum target, GLenum internalformat, GLuint buffer) {
    ...
    // PRIORIDADE: Usar GL_EXT_texture_buffer nativo
    if (mg_texture_buffer_available() && g_tex_buffer_ext) {
    ...
    if (hardware->emulate_texture_buffer) {
        ...
        if (pixelSize == 0) {
            BU_WARN_ONCE("glTexBuffer: no texel size known for internalformat %s",
        ...
        if (!get_internal_format_transfer(internalformat, &tb_format, &tb_type)) {
            BU_WARN_ONCE("glTexBuffer: no GLES transfer pair for internalformat %s",
                         glEnumToString(internalformat));
            mg_set_gl_error(GL_INVALID_ENUM);
            return;
        }
        ...
        if (numElements == 0) {
            BU_WARN_ONCE("glTexBuffer: buffer of %d bytes holds no %u-byte texel", bufferSize, pixelSize);
